...
1.  **Mở các file migration và cập nhật Tên User:**
    -   **Đây là bước quan trọng để tránh lỗi "permission denied".**
    -   Mở các file migration SQL trong thư mục `backend/migrations/`.
...
3.  **Chạy các file Migration cần thiết:**
...
2.  Bên trong phiên `screen`, chạy các lệnh sau để yêu cầu báo cáo cho 60 ngày qua (thay đổi ngày tháng nếu cần):
    ...
    npm run request:sb-search-terms -- 2025-09-01 2025-09-17

    # Yêu cầu báo cáo Sponsored Display
    npm run request:sd-targeting -- 2025-08-01 2025-09-19
    ```
...
2.  **Thêm các dòng Cron Job:**
    ...
    -   **CỰC KỲ QUAN TRỌNG:** Hãy thay đổi `/var/www/Enkezi-Ads-Auto` thành đường dẫn **CHÍNH XÁC** đến thư mục dự án của bạn và đảm bảo dòng `PATH` là chính xác.
    ...
    5 1 * * * cd /var/www/Enkezi-Ads-Auto && npm run request:sb-search-terms -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Enkezi-Ads-Auto/logs/cron_sb_search_term_request.log 2>&1
    8 1 * * * cd /var/www/Enkezi-Ads-Auto && npm run request:sd-targeting -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Enkezi-Ads-Auto/logs/cron_sd_targeting_request.log 2>&1
    
    # Tự động lấy báo cáo Sales & Traffic hàng ngày lúc 1:10 AM cho dữ liệu của 2 ngày trước
    10 1 * * * cd /var/www/Enkezi-Ads-Auto && npm run fetch:sales-traffic -- $(date -d "2 days ago" +\%Y-\%m-\%d) $(date -d "2 days ago" +\%Y-\%m-\%d) >> /var/www/Enkezi-Ads-Auto/logs/cron_sales_traffic.log 2>&1
...
- **Nguyên nhân:** Lỗi này xảy ra khi người dùng database được định nghĩa trong `backend/.env` không có quyền đọc/ghi trên bảng mà script đang cố gắng truy cập.
- **Giải pháp:**
